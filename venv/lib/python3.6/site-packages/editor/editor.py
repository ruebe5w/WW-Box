...
class SvgDraggablePoint(gui.SvgRectangle, DraggableItem):
    ...
    def update_position(self):
        if self.refWidget:
            self.set_position(float(self.refWidget.attributes[self.name_coord_x]) - self.w / 2,
...
class SvgDraggableRectangleResizePoint(gui.SvgRectangle, DraggableItem):
    ...
    def update_position(self):
        if self.refWidget:
            self.set_position(
                float(self.refWidget.attributes['x']) + float(self.refWidget.attributes['width']) - self.w / 2,
                float(self.refWidget.attributes['y']) + float(self.refWidget.attributes['height']) - self.h / 2)
...
class SvgDraggableCircleResizeRadius(gui.SvgRectangle, DraggableItem):
    ...
    def update_position(self):
        if self.refWidget:
            self.set_position(
                float(self.refWidget.attributes['cx']) + float(self.refWidget.attributes['r']) - self.w / 2,
        ...
        if self.refWidget:
            if 'left' in self.refWidget.style and 'top' in self.refWidget.style:
                self.style['left'] = gui.to_pix(
                    gui.from_pix(self.refWidget.style['left']) + gui.from_pix(self.refWidget.style['width']))
                self.style['top'] = gui.to_pix(
                    gui.from_pix(self.refWidget.style['top']) + gui.from_pix(self.refWidget.style['height']))
...
class DragHelper(gui.Widget, DraggableItem):
    ...
    def update_position(self):
        ...
        if self.refWidget:
            if 'left' in self.refWidget.style and 'top' in self.refWidget.style:
                self.style['left'] = gui.to_pix(
                    gui.from_pix(self.refWidget.style['left']) - gui.from_pix(self.style['width']))
                self.style['top'] = gui.to_pix(
                    gui.from_pix(self.refWidget.style['top']) - gui.from_pix(self.style['width']))
...
class Project(gui.Widget):
        ...
        for (setOnEventListenerFuncname, setOnEventListenerFunc) in inspect.getmembers(widget):
            ...
            if hasattr(setOnEventListenerFunc, '_event_info'):
                ...
                if getattr(widget, setOnEventListenerFuncname).callback:
                    getattr(widget, setOnEventListenerFuncname).callback_copy = getattr(widget,
                                                                                        setOnEventListenerFuncname).callback
    ...
    def check_pending_listeners(self, widget, widgetVarName, force=False):
        ...
        for event in self.pending_listener_registration:
            ...
            if force or (hasattr(event['eventsource'], 'path_to_this_widget') and hasattr(event['eventlistener'],
                                                                                          'path_to_this_widget')):
                if (force or (widget.attributes['editor_varname'] in event['eventsource'].path_to_this_widget and
                              widget.attributes['editor_varname'] in event['eventlistener'].path_to_this_widget)) and \
                        event['done'] == False:
                    ...
                    if force or (
                            self.children['root'] == widget and not (widget.attributes['editor_newclass'] == 'True')):
                    ...
                    if len(listener_filtered_path) > 0:
                        listenername = ("%s.children['" + "'].children['".join(
                            listener_filtered_path) + "']") % listenername
    ...
    def repr_widget_for_editor(self, widget):  # widgetVarName is the name with which the parent calls this instance
        ...
        code_nested = prototypes.proto_widget_allocation % {'varname': widgetVarName, 'classname': classname,
                                                            'editor_constructor': widget.attributes[
                                                                'editor_constructor'],
                                                            'editor_instance_id': widget.identifier}

        code_nested += prototypes.proto_attribute_setup % {'varname': widgetVarName, 'attr_dict': ','.join(
            '"%s":"%s"' % (key, widget.attributes[key]) for key in widget.attributes.keys() if
            key not in html_helper.htmlInternallyUsedTags)}

        code_nested += prototypes.proto_style_setup % {'varname': widgetVarName, 'style_dict': ','.join(
            '"%s":"%s"' % (key, widget.style[key]) for key in widget.style.keys())}
        ...
        for (setOnEventListenerFuncname, setOnEventListenerFunc) in inspect.getmembers(widget):
            ...
            if hasattr(setOnEventListenerFunc, '_event_info'):
                ...
                if hasattr(getattr(widget, setOnEventListenerFuncname), 'callback_copy'):
                    ...
                    listenerFunctionName = setOnEventListenerFunc._event_info['name'] + "_" + widget.attributes[
                        'editor_varname']
        ...
        for child_key in widget.children.keys():
            ...
            children_code_nested += prototypes.proto_layout_append % {'parentname': widgetVarName,
                                                                      'varname': "%s,'%s'" % (
                                                                      child.attributes['editor_varname'],
                                                                      child.attributes['editor_varname'])}
        ...
        if newClass:  # and not (classname in self.code_declared_classes.keys()):
            ...
            self.code_declared_classes[widget.identifier] = prototypes.proto_code_class % {'classname': classname,
                                                                                           'superclassname':
                                                                                               widget.attributes[
                                                                                                   'editor_baseclass'],
    ...
    def save(self, save_path_filename, configuration):
        ...
        self.known_project_children = [
            self, ]  # a list containing widgets that have been parsed and that are considered valid listeners
        ...
        code_nested = ret + self.check_pending_listeners(self, 'self',
                                                         True)  # + self.code_listener_registration[str(id(self))]
        ...
        compiled_code = prototypes.proto_code_program % {'code_classes': code_classes,
                                                         'classname': configuration.configDict[
                                                             configuration.KEY_PRJ_NAME],
...
class Editor(App):
    ...
    def main(self):
        ...
        self.fileOpenDialog = editor_widgets.EditorFileSelectionDialog('Open Project',
        ...
        self.fileSaveAsDialog = editor_widgets.EditorFileSaveDialog('Project Save',
        ...
        self.projectConfiguration = editor_widgets.ProjectConfigurationDialog('Project Configuration',
