...
class SignalConnection(gui.Widget):
    def __init__(self, widget, listenersList, eventConnectionFuncName, eventConnectionFunc, **kwargs):
        ...
        self.set_layout_orientation(gui.Widget.LAYOUT_HORIZONTAL)
        self.style.update(
            {'overflow': 'visible', 'height': '24px', 'display': 'block', 'outline': '1px solid lightgray'})
        self.label = gui.Label(eventConnectionFuncName, width='49%')
        self.label.style.update(
            {'float': 'left', 'font-size': '10px', 'overflow': 'hidden', 'outline': '1px solid lightgray'})
        ...
        if hasattr(self.eventConnectionFunc,
                   'callback_copy'):  # if the callback is not None, and so there is a listener
    ...
    def on_connection(self, widget, dropDownValue):
        if self.dropdown.get_value() == 'None':
            ...
            print("Event: " + self.eventConnectionFuncName + " signal connection to: " + listener.attributes[
                'editor_varname'] + "   from:" + self.refWidget.attributes['editor_varname'])
            ...
            getattr(self.refWidget, self.eventConnectionFuncName).callback_copy = getattr(self.refWidget,
                                                                                          self.eventConnectionFuncName).callback
...
class ProjectConfigurationDialog(gui.GenericDialog, gui.EventSource):
    ...
    def __init__(self, title='', message=''):
        ...
        self.add_field_with_label(self.KEY_MULTIPLE_INSTANCE, 'Use single App instance for multiple users',
                                  gui.CheckBox(True))
...
class WidgetHelper(gui.HBox):
    ...
    def on_dialog_confirm(self, widget):
        ...
        if variableName in self.varname_list:
            self.errorDialog = gui.GenericDialog("Error",
                                                 "The typed variable name is already used. Please specify a new name.",
        ...
        widget.attributes.update({'editor_constructor': constructor,
                                  ...
                                  'editor_newclass': 'True' if self.dialog.get_field(
                                      "editor_newclass").get_value() else 'False',
...
class WidgetCollection(gui.Widget):
    def __init__(self, appInstance, **kwargs):
        ...
        self.add_widget_to_collection(gui.Link, width='100px', height='30px',
                                      style={'top': '20px', 'left': '20px', 'position': 'absolute'})
        self.add_widget_to_collection(gui.Progress, width='130px', height='30px',
                                      style={'top': '20px', 'left': '20px', 'position': 'absolute'})
...
class EditorAttributes(gui.VBox, gui.EventSource):
        ...
        for attribute in html_helper.editorAttributeList:
            ...
            if not attributeValue['group'] in self.attributeGroups.keys():
                ...
                groupContainer.style['-webkit-order'] = str(
                    html_helper.editorAttributesGroupOrdering[attributeValue['group']])
    ...
    def onattribute_changed(self, widget, widgetAttributeMember, attributeName, newValue):
        print("setting attribute name: %s    value: %s    attributeMember: %s" % (
    ...
    def __init__(self, attributeName, attributeDict, appInstance=None):
        ...
        if attributeDict['type'] in (bool, int, float, gui.ColorPicker, gui.DropDown, 'url_editor', 'css_size'):
            ...
            if attributeDict['type'] == int or attributeDict['type'] == float:
                self.inputWidget = gui.SpinBox(attributeDict['additional_data']['default'],
